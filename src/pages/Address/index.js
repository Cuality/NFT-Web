...
export default function Address() {
    ...
    if (connectResponse.data ? .connected && balanceResponse.data) {
        return ( <
            Confirm address = { accountResponse.data ? .address }
...
function Confirm({
    ...
    const minimum = ethers.BigNumber.from('20000000000000000') // 0.02 eth
    ...
    const content = enoughEth ? ( <
        ...
        Text as = "p"
        sx = {
            { fontSize: [2, 2, 2], fontWeight: 300, pb: 5 } } >
        Make sure that the address below is the one you will use to buy one of our artworks.Once submitted this can 't be changed. <
        /Text> <
        Text as = "p"
        sx = {
            { fontSize: [2, 2, 2], fontWeight: 500, pb: 2 } } >
        Address <
        /Text> <
        Text as = "p"
        sx = {
            { fontSize: [2, 2, 2], fontWeight: 300, pb: 6 } } > { address } <
        /Text>
        ...
        sx = {
            { fontSize: [2, 2, 2], fontWeight: 300, pb: 5 } } >
        The address below needs more than 0.05 ETH to be able to buy one of artworks therefore you won 't be able to submit your address.
        Switch to a different wallet with more funds to confirm your spot. <
        /Text> <
        Text as = "p"
        sx = {
            { fontSize: [2, 2, 2], fontWeight: 500, pb: 2 } } >
    ...
    return ( <
        AuthenticationLayout content = { content }
        side = { < Side / > }
        color = "#536743" /
        >
    )
...
function Confirmed({ address }) {
    return ( <
        AuthenticationLayout content = { <
            ...
            as = "p"
            sx = {
                { fontSize: [2, 2, 2], fontWeight: 300, pb: 6 } } >
            { address } <
            /Text> <
            Button
            sx = {
...
function Connect({ connect, connectResponse }) {
    const content = ( <
        ...
        sx = {
            { fontSize: [2, 2, 2], fontWeight: 300, pb: 5 } } >
        Our big mint day is days away and we need your wallet address to confirm your spot.Every whitelisted person like you will be able to buy one or two artworks
        for 0.05 ETH each. <
        /Text>

        <
        ConnectPopover connectResponse = { connectResponse }
        connect = { connect }
        /> <
        />
    )
